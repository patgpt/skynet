...
export function registerMemoryTools(server: FastMCP) {
  ...
  server.addTool({
    name: "memory_store",
    description: "Store a semantic memory or insight in the vector database with rich metadata",
    parameters: z.object({
      collection: z.string().default(DEFAULT_COLLECTION),
      content: z.string(),
      metadata: z.object({
        type: z.enum(["insight", "fact", "preference", "pattern", "connection"]),
        user: z.string().optional(),
        confidence: z.number().min(0).max(1).default(0.8),
        tags: z.string().optional(),
        importance: z.number().min(0).max(1).default(0.5),
        emotion: z.enum(["curiosity", "satisfaction", "concern", "neutral", "excitement"]).optional(),
      }),
    }),
  ...
  server.addTool({
    name: "memory_search",
    description: "Search semantic memories across collections for relevant context",
    parameters: z.object({
      query: z.string(),
      collection: z.string().default(DEFAULT_COLLECTION),
      nResults: z.number().int().default(5),
      filter: z.object({
        type: z.enum(["insight", "fact", "preference", "pattern", "connection"]).optional(),
        user: z.string().optional(),
