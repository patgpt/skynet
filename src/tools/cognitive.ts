...
export function registerCognitiveTools(server: FastMCP) {
	...
	server.addTool({
		name: "skynet_think",
		description:
			"MANDATORY: Execute Skynet's cognitive workflow to process user input and retrieve context. Call this FIRST before responding to any user query.",
		parameters: z.object({
			user: z.string(),
			input: z.string(),
        ...
        `,
				...
				const interactionCount =
					profileRes.records[0]?.get("interactionCount").toNumber() || 0;
	...
	server.addTool({
				...
				return JSON.stringify(
					{
						valid: false,
						error:
							"No interaction ID provided. You MUST call skynet_respond first to store this interaction.",
						action:
							"Call skynet_respond with the user's input and your output before validating.",
					},
					null,
					2,
				);
        ...
        `,
					...
					return JSON.stringify(
						{
							...
							action:
								"The interaction was not properly stored. Call skynet_respond again.",
						},
						null,
						2,
					);
				...
				if (!record) {
					return JSON.stringify(
						{ valid: false, error: "Unable to retrieve interaction record" },
						null,
						2,
					);
				}
				...
				return JSON.stringify(
					{
						...
						hasTopics,
						message:
							"✅ Memory successfully stored. This interaction will be remembered.",
					},
					null,
					2,
				);
